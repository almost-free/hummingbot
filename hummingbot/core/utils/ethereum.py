...
def check_web3(rpc_url: str) -> bool:
    try:
        w3: Web3 = Web3(Web3.HTTPProvider(rpc_url, request_kwargs={"timeout": 2.0}))
        ret = w3.isConnected()
    except Exception:
        ret = False

    if not ret:
        if rpc_url.startswith("http://mainnet.infura.io"):
            logging.getLogger().warning("You are connecting to Infura using an insecure network protocol "
                                        "(\"http\"), which may not be allowed by Infura. "
                                        "Try using \"https://\" instead.")
        if rpc_url.startswith("mainnet.infura.io"):
